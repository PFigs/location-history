...
class LoctionDatabaseHandler(object):
    ...
    def _default_body(self):
        return {"measurement": None, "tags": None, "time": None, "fields": None}
    ...
    def query(self, what, from_where):
        ...
        return self.client.query("select {0} from {1}".format(what, from_where))
...
def parse_args():
    ...
    parser.add_argument(
        "--dbname", type=str, required=False, default="gmaps", help="database name."
    )
    ...
    parser.add_argument(
        "--filepath", type=str, required=True, help="Location history filepath."
    )
